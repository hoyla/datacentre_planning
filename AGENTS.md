...
| A report, count or reader panel that looks wrong | the runbook's **"Expected, and not a fault — do not investigate these again"** section FIRST. Each entry there was taken apart once and the answer written down; anything not listed there is new |
...
| Tests, CI, or the test database | the header comments of `tests/conftest.py` and `.github/workflows/checks.yml`, which say what each job runs and what each marker means; [docs/TEST_SUITE_REVIEW_2026-09-16.md](docs/TEST_SUITE_REVIEW_2026-09-16.md), the review that produced the 2026-09-16 fixes, for what the suite covered and did not that day, and ROADMAP "From the test-suite review" for what is still to write |
