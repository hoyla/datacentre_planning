...
re-run rather than copied: `scripts/computed_figures.py` reads every
...
aggregates and Sources tables in the reader; he had not — they were
workbook-only, and the methodology page cited three of the five
external sources in passing and never the word Provenance. A reporter
working from the reader alone could not see the two UK Power Networks
datasets the release rests on, or where any external figure came from,
which cut against the rule the rest of the page keeps.

The methodology page now carries a "Sources outside the planning
record" table — title, publisher, date, locator with the date it was
read, and each source's own limits — generated by
`export_reader.external_sources_html` from
`dcp/external_aggregates.SOURCES`, the table the workbook's Provenance
sheet is written from, so the two surfaces cannot list different
sources. A test asserts every source's title, locator and access date
reach the page. Deferred past 2.10 because the artefacts were built
and diffed when it surfaced; done while 2.15 accumulates.
