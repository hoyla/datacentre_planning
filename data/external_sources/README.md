...
## ea-industrial-installations.zip, ea-permit-claims.yaml, ea-permit-documents.json
...
Two committed files stand in for them. `ea-permit-claims.yaml` holds the
42 claims, each with the verbatim sentence it was read from, so a
published figure is checkable from a clone with no documents in it; it is
generated by `scripts/read_ea_permits.py --write-claims` and should not
be hand-edited. `ea-permit-documents.json` is the manifest — the
publication path, and for each document its title, URL, sha256, size and
page count — which is what makes the PDFs re-fetchable and lets a
