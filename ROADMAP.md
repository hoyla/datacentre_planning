...
sites**, **2,022 applications** in the site universe, **63,502
documents** (documents re-measured 2026-09-16). Four sites are new — three from the site 11 partition and
...
The reviews and decisions only he can make, in one place because the
corpus now moves faster than he can read it (his words, 2026-09-10).
Each row's detail is in the section it names; a row leaves this table
when the thing is done, and a new item that waits on him joins it in
the same PR that creates it. Sizes are measured when written, not
maintained — re-count before quoting.

| what | where | size | since | what it unblocks |
|---|---|---|---|---|
| The C and D computed figures, one row each with the quote and our copy of the page | `data/computed_figures_review/computed_figures_review.xlsx`, regenerated by `scripts/computed_figures.py --review` (#248 below) | 62 rows; 19 are the figure a site's box shows | 2026-09-10 | the fold of each decision into a person's adjudication row; the write-time guard is already in force (#432) |
| The campus-scope review, one campus at a time, with the assistant | `data/priors/campus_scope.yaml`; "How to continue the 35-campus review" below | 30 of 34 `unreviewed` | 2026-08-31 | the partitions the review owes and #247's general case |
...
| UKPN's "Data Centres by Local Authority", behind his portal login | "UKPN's \"Data Centres by Local Authority\" is unpulled" below | 1 download | 2026-08 | a per-authority count to set beside the consumption context |
...
| The model one-liners | [docs/MODELS.md](docs/MODELS.md), "Open" | 3 lines: the `--model` default, a durable reading model, `gpt-5.6-sol` untested | 2026-08-31 | the runbook stops saying "pass `--model gpt-5` every time" |
| Whether `dcp/reader.py` and `dcp/export.py`, the phase-1 path, stay in the tree | \"From the test-suite review\" below | 1 decision | 2026-09-16 | deleting two thousand lines nothing published runs on, or keeping them on purpose |
| Step 15, telling the reporting team; and at 2.15, carrying the Sheet annotations keyed on the twelve old site keys across by hand | "Regenerating a release" and "Outstanding from the figure-standing work" below | 1 message; 12 sites' rows | 2026-09-07 | the team knowing 2.14 exists; annotations surviving the re-key |
...
all 29 read in full, and 390 of 390 corpus-wide since 2026-09-10
(HISTORY, "The last prose gap closes"). Two things it surfaced are still open, and one is
...
- **The standard the next test is written to** (from the older
  test-surface item this section replaced): `tests/test_release_defaults.py`
  asserts a *rule* over the whole tree and was verified by reintroducing
  the bug; `tests/test_adjudication_gate.py` is the counter-example,
  asserting the corrector and the gate agree with nothing asserting
  either is right. Two traps: a determinism check has to run at scale or
  read the query, and a bound fitted to whatever the code returned tests
  nothing.
- **`dcp/reader.py` and `dcp/export.py`** are the phase-1 path; the
  honest question is whether they should still be in the tree (a row in
  the table above).
...
- **The `not_dc` live-member leak is neither small nor about verdict
  ...
  clusterer's own rubric fold; the numbers that follow are that day's,
  and made the case for migration 034 — at the 2026-09-10 materialise
  the rule excludes **260** live members, 183 by verdict and 77 as the
  paperwork of one, admits 2, six sites are keyed on an excluded
  member, Kingsnorth is not among them because `Medway/MC/21/0979` is
  admitted in `not_dc_standing.yaml`, and neither the British Museum
  nor the Gardiner Haskins figure stands on a site any more).
  **159 live members** carried a latest
  ...
  **The figure half shipped, 2026-09-10** (migration 034; HISTORY, "A
  ...
  moved on the power columns, listed in HISTORY. Luke confirmed the
  two admissions on 2026-09-10; the file records it beside the evidence.
...
- **The acquisition tail, as re-measured 2026-09-16: 189 in-universe
  applications hold no documents** (290 on 2026-08-30). By latest
  `acquisition_outcome`: **90 `no_adapter`**, **58 `none_published`**
  (the register lists nothing — the refused-page class in "Acquisition
  decisions waiting on a person" sits inside this), **30 `error`**
  (retryable — re-running the fetch picks them up), **7
  `portal_blocked`**, **3 `login_required`** and one never-attempted. The
...
1. **Sites 59 and 5 still block the Premier Park and DataVita
   ...
   on 2026-08-27.
2. **Thirteen names could not be resolved to a company**, listed under
...
- **About twenty documents the corpus extraction cannot cache**, the
  residue the read-in-full item always named, none of it holding a site
  back (the gap itself closed 2026-09-10; re-measure through
  `site_profile.load_coverage_detail` rather than quoting it): two
  `.xlsb`, a `.ppt`, five of no recognisable format, the PDFs pypdf
  refuses to open — six joined on 2026-09-10 — and four Central
...
**Two councils serve a listed document as zero bytes, and it is worth
telling them.** `005 - Section 106 Agreement.pdf` on Wakefield
23/00100/S7301 (Ferrybridge C) and `011 - Consultation Response.pdf`
on Warwick W/23/1025 (Warwick Hospital) return HTTP 200,
`application/pdf` and an empty body from the councils' own servers,
with a session cookie and referer; Luke confirmed the same in a
browser. Medway's `018 - Supporting Documents.pdf` on MC/21/0979 was
the register listing one document twice, and the second copy is held.
The pipeline's side is done — the guard at `repo.record_document`, the
every-release sweep in `repo.zero_byte_files`, the coverage split's
`empty` bucket, and since 2026-09-15 the queue's retry of the empty
file on every run through `repo.held_bytes` (HISTORY, "An empty
document is not held") — so what remains is a public-access failure
...
- **A public feed: Barbour-stripped site summaries as GeoJSON, with a
  ...
  application — 482 of the 503 live sites do (2026-09-16); 21 are Barbour-only and
...
- **The reader's weight is its inline site pages, and the one-file rule
  ...
  `index.html` is 33.5 MB (35.6 MB at 2.14); the sites view is 29.3 MB, of which the 508
...
- **Requests outstanding, and three drafted awaiting Luke's send.**
  ...
  sending remains.
- **UKPN's "Data Centres by Local Authority" is unpulled.** It sits
  behind Luke's portal login; anonymous access returns headers only, so
  nobody else can fetch it. The Large Demand List and the Data Centre
  Demand Profiles were pulled the same way on 2026-08-12 and are in
  `data/external_sources/`, registered in `dcp/external_aggregates.py`.
