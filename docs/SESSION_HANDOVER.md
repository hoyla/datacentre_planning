...
register snapshot (`ea-industrial-installations.zip`, a zip despite the
URL), because it is regenerated daily with no archive behind it and
yesterday's is simply gone. `ea-permit-claims.yaml`, generated by
...
`ea-permit-documents.json`, pinning every document's URL, sha256, byte
